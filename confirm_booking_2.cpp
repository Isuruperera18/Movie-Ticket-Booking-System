...
#include "ui_confirm_booking_2.h"

Confirm_Booking_2::Confirm_Booking_2(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Confirm_Booking_2)
{
    ui->setupUi(this);
}

Confirm_Booking_2::~Confirm_Booking_2()
{
    delete ui;
}


void Confirm_Booking_2::systemFunction()
{
    ui->cmbConfirm->clear();
    ...
    if(ui->cmbConfirm->count()<0)
        ui->btnConfirm2->setEnabled(false);
}


void Confirm_Booking_2::on_btnConfirm2_clicked()
{
    QString seat = ui->cmbConfirm->currentText();

    QSqlQuery query(System_Database::getInstance()->getSystemDBInstance());
    query.prepare("update Seat_Availability_2 set Available='R' where Seat_Number= '" + seat +"'"); // update seats availability to reserved
