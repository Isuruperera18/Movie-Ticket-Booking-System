...
#include "ui_cancel_seat_1.h"

Cancel_Seat_1::Cancel_Seat_1(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Cancel_Seat_1)
{
    ui->setupUi(this);
}

Cancel_Seat_1::~Cancel_Seat_1()
{
    delete ui;
}


void Cancel_Seat_1::systemFunction()
{
    ui->cmbCancel->clear();
...
void Cancel_Seat_1::on_btnCancel1_clicked()
{
    QString seat = ui->cmbCancel->currentText();

    QSqlQuery query(System_Database::getInstance()->getSystemDBInstance());
    query.prepare("update Seat_Availability_1 set Available='T' where Seat_Number= '" + seat +"'"); // update seats availability to available
